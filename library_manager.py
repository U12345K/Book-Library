...
lottie_book = load_lottieurl("https://assets9.lottiefiles.com/temp/1f20_aKfIn.json")
if lottie_book:
...
if st.session_state.current_view == "add":
    ...
    with st.form(key='add_book_form'):
        ...
        with col2:
            genre = st.selectbox("Genre",options=["Fiction","Non-Fiction","Science Fiction","Fantasy","Mystery","Romance","Thriller","Horror","Biography","History","Self-Help","Business","Technology","Travel","Cooking","Art","Poetry","Drama","Comedy","Action","Adventure","Animation","Children's","Young Adult","Crime","Suspense","Western","War","Musical","Western","War","Musical","Documentary","Short Film","Television","Video","Film","Video Games","Other"])
            read_status = st.radio("Read Status",["Read","Unread"],horizontal=True)
            read_bool = read_status == "Read"
            submit_button = st.form_submit_button(label="Add Book")
            
        if submit_button and title and author:
            read_book(title, author, publication_year, genre, read_bool )
    ...
    if not st.session_state.Library:
        ...
        for i, book in enumerate(st.session_state.Library):
            with cols[i % 2]:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("Remove", key=f"remove_button_{i}",use_container_width=True):
                        if remove_book(i):
                            st.rerun()
                with col2:
    ...
    if not st.session_state.Library:
        st.markdown(
            """
            <div class="warning-message">
                <p>Your library is empty. Add some books to see statistics.</p>
            </div>
            """,
            unsafe_allow_html=True)
    else:
        ...
        with col3:
            st.metric("Percent Read", f"{stats['percent_read']:.if}%") 
        ...
        if stats['genres']:
            st.markdown("<h3>Top Genres</h3>", unsafe_allow_html=True)
            top_genres = dict(list(stats['genres'].items())[:5])
